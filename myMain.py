...
# net.load_state_dict(torch.load('checkpoint/ENet_0.841_equal_True.pth',map_location=map_location))

optimiser = torch.optim.Adam([
    {'params': net.module.encoder.parameters(), 'lr': lr / 4},
    {'params': net.module.decoder.parameters()}
], lr=lr, weight_decay=weigth_decay)
## loss
...
def val(net, dataloader_):
    ...
    if highest_iou < iou_meter_val.value()[0]:
        ...
        torch.save(net.state_dict(),
                   'checkpoint/modified_ENet_%.3f_%s.pth' % (iou_meter_val.value()[0], 'equal_' + str(Equalize)))
