...
criterion_ce = CrossEntropyLoss2d(class_weigth).cuda() if (torch.cuda.is_available() and use_cuda) else CrossEntropyLoss2d(class_weigth)
...
def train():
    ...
    for epoch in range(max_epoch):
        ...
        for i,(img,mask,_) in tqdm(enumerate(train_loader)):
            ...
            fake_loss = criterion_bce(fake_score,torch.zeros((batch_size),requires_grad=False).cuda())
            fake_loss.backward(retain_graph=True)

            real_score = discriminator(img,int2onehot(mask).cuda())
            real_loss = criterion_bce(real_score,torch.ones((batch_size),requires_grad=False).cuda())
            real_loss.backward()

            d_optimiser.step()

            loss_s = criterion_ce(pred,mask.squeeze(1)) + 0.1* criterion_bce(fake_score, torch.ones((batch_size),requires_grad=False).cuda())
            loss_s.backward()
            s_optimiser.step()
